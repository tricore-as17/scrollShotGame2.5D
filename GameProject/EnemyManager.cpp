...
{
    //それぞれの敵の座標を代入していく
	easyEnemy[0]->Initialize(VGet(EAZY1_FIRST_X, EAZY1_FIRST_Y, 0));
    easyEnemy[1]->Initialize(VGet(EAZY2_FIRST_X, EAZY1_FIRST_Y, 0));
...
void EnemyManager::Update(const Map& map, const VECTOR& cameraPosition, ShotManager& shotManager)
{
    ...
    SingleUpdate(easyEnemy, map, cameraPosition, shotManager);
    SingleUpdate(leftShotEnemy, map, cameraPosition, shotManager);
    //体力の値によって削除する
...
void EnemyManager::SingleUpdate(vector<BaseEnemy*> enemy, const Map& map, const VECTOR& cameraPosition, ShotManager& shotManager)
{
