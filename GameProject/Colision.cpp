    ...
    while (loop)
    {
        ...
        for (int hIndex = 0; hIndex < mapYNum; hIndex++)
        {
            ...
            for (int wIndex = 0; wIndex < mapXNum; wIndex++)
            {
                ...
                VECTOR mapChipPos = VAdd(VGet(Map::CHIP_SIZE * wIndex + mapLeftPos.x, Map::CHIP_SIZE * (-hIndex - 1) + mapLeftPos.y, 0), Map::CHIP_HALF_OFFSET);
                //オブジェクトとマップの当たり判定をみる
                isHit =IsHitRectangles(pos, w,h, mapChipPos,Map::CHIP_SIZE,Map::CHIP_SIZE);
...
bool Colision::CheckIsGround(const Map& map, const VECTOR& objectPos,const float objectW,const float objectH)
{
    //接地判定用のフラグ
    bool isGround = false;
    //横と縦のマップチップの数を持ってくる
